...
export default function Messages() {
  ...
  // -------- Make Section 2 live: listen to BOTH messages INSERT and conversations UPDATE --------
  useEffect(() => {
    const u = userRef.current
    if (!u) return

    // Any message involving me (not necessarily the open one)
    const msgCh = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          // If it’s not the currently open conversation, refresh Section 2 list (badges + order)
    ...
    // Also listen to conversations updated_at (triggered after each message)
      ...
      .on(
        ...
        (payload) => {
          ...
          if (row.participant1 === u.id || row.participant2 === u.id) {
            // keeps Section 2 live-sorted without refresh
            fetchConversations()
          }
        }
      )
    ...
    // Light polling safety net (every 4s)
                  ...
                  <div className="p-4 border-t border-gray-200">
                    <div className="flex space-x-2">
                      <input
                        ...
                        disabled={sending}
                        // mobile keyboard + autofill fixes:
                        autoComplete="off"
                        autoCorrect="off"
