...
export default function Home() {
  ...
  const applyFilters = () => {
    ...
    // Price filtering - only filter listings with matching currency
    filtered = filtered.filter(listing => {
      if (listing.currency !== filters.currency) return false
      const price = listing.price
      const minPrice = filters.minPrice || 0
      const maxPrice = filters.maxPrice || (filters.currency === 'USD' ? 5000 : 5000000)
  ...
  const clearFilters = () => {
    setFilters({
      city: '',
      minPrice: 0,
      maxPrice: filters.currency === 'USD' ? 5000 : 5000000,
      rooms: '',
      currency: 'USD',
      searchQuery: ''
    })
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        ...
                        setFilters(prev => ({ 
                          ...
                          minPrice: 0,
                          maxPrice: newCurrency === 'USD' ? 5000 : 5000000
                        }))
                      }}
                      className="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:ring-4 focus:ring-blue-500/20 focus:border-blue-500 transition-all bg-white"
                    >
                    ...
                    <div className="space-y-4">
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            setFilters(prev => ({ 
                              ...prev, 
                              minPrice: newMin,
                              maxPrice: Math.max(newMin, prev.maxPrice)
                            }))
                          }}
                          className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer price-slider-min"
                        />
                        <div className="text-center text-sm font-semibold text-blue-600 mt-1">
                          {filters.minPrice.toLocaleString()} {filters.currency}
                        </div>
                      </div>
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            setFilters(prev => ({ 
                              ...prev, 
                              maxPrice: newMax,
                              minPrice: Math.min(prev.minPrice, newMax)
                            }))
                          }}
                          className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer price-slider-max"
                        />
                        <div className="text-center text-sm font-semibold text-blue-600 mt-1">
                          {filters.maxPrice.toLocaleString()} {filters.currency}
                        </div>
                      </div>
                      ...
                      <div className="bg-blue-50 rounded-lg p-3 text-center">
                        ...
                        <div className="font-bold text-blue-700">
                          {filters.minPrice.toLocaleString()} - {filters.maxPrice.toLocaleString()} {filters.currency}
                        </div>
                      </div>
                    </div>
                  </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {(filters.city || filters.rooms || filters.maxPrice) && (
            <div className="mb-6 flex flex-wrap items-center gap-2 justify-center">
              ...
              {(filters.minPrice > 0 || filters.maxPrice < (filters.currency === 'USD' ? 5000 : 5000000)) && (
              ...
              {viewMode === 'map' && (
                <div className="bg-gradient-to-r from-green-50 to-emerald-50 rounded-2xl p-6 border border-green-200">
                  <div className="text-center mb-4">
                    <div className="text-2xl font-bold text-green-700 mb-2">🗺️ Map View</div>
                    <p className="text-gray-600">Interactive property locations</p>
                  </div>
                  <MapView 
