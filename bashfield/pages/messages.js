...
export default function Messages() {
  ...
  const { translateMessage } = useAutoTranslate()
  ...
  const fetchMessages = useCallback(async (conversationId) => {
    ...
    const merged = await Promise.all(
      msgs.map(async (m) => {
        const translatedMessage = await translateMessage(m)
        return {
          ...translatedMessage,
    ...
    const ch = supabase
      ...
      .on(
        ...
        async (payload) => {
          ...
          const translatedMessage = await translateMessage(m)
          setMessages((prev) => (prev.some((x) => x.id === m.id) ? prev : [...prev, { ...translatedMessage, sender: profile }]))
