...
import { saveLanguagePreference, getLanguagePreference } from '../lib/languageService'
...
export default function Layout({ children }) {
  ...
  useEffect(() => {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(async (event, session) => {
      if (session?.user) {
        ...
        await loadUserLanguage(session.user)
      } else {
        setUser(null)
        setProfile(null)
        await loadUserLanguage(null)
      }
    })
  ...
  const loadUserLanguage = async (user) => {
    try {
      const preferredLang = await getLanguagePreference(user?.id)
      if (preferredLang !== router.locale) {
        // Only redirect if language is different and we haven't loaded yet
        if (!languageLoaded) {
          const { pathname, asPath, query } = router
          await router.replace({ pathname, query }, asPath, { locale: preferredLang })
        }
      }
  ...
  // Language change with database persistence
  const changeLanguage = async (lng) => {
    if (lng === router.locale) return
    
    try {
      // Save preference immediately
      await saveLanguagePreference(lng, user?.id)
            ...
            <div className="flex items-center">
              <div className="flex items-center space-x-4 cursor-pointer group" onClick={() => router.push('/')}>
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500 hidden sm:block">{safeT('ui.premierPlatform', 'Premier Property Platform')}</p>
                </div>
              </div>
            </div>
              ...
              {user && (
                <>
                  ...
                  >
                    ...
                    <span>{safeT('nav.addProperty', 'Add Property')}</span>
                  </button>
                  <button 
                  ...
                  >
                    ...
                    <span>{safeT('nav.favorites', 'Favorites')}</span>
                  </button>
                </>
              )}
              {isAdmin && (
                ...
                >
                  ...
                  <span>{safeT('nav.admin', 'Admin')}</span>
                </button>
              )}
            </div>
            ...
            <div className="hidden lg:flex items-center space-x-2">
              {user && (
                ...
                >
                  ...
                  <span>{safeT('nav.messages', 'Messages')}</span>
                  {unreadCount > 0 && (
              ...
              {loading ? (
                ...
                <div className="flex items-center space-x-4">
                  ...
                  >
                    ...
                    <div className="hidden lg:block">
                      ...
                      <div className="text-xs text-gray-500">{safeT('nav.viewProfile', 'View Profile')}</div>
                    </div>
                  </div>
                  <button 
                  ...
                  >
                    ...
                    <span>{safeT('nav.logout', 'Logout')}</span>
                  </button>
                </div>
              ) : (
                <button 
                ...
                >
                  ...
                  <span>{safeT('nav.signIn', 'Sign In')}</span>
                </button>
              )}
            </div>
            ...
            <div className="lg:hidden border-t border-gray-200/50 py-6 bg-gradient-to-b from-white to-gray-50">
              <div className="space-y-2 px-4">
                ...
                {user && (
                  <>
                    <button 
                      onClick={() => { router.push('/post'); setMobileMenuOpen(false) }} 
                      className={`flex items-center space-x-3 w-full text-left px-4 py-3 rounded-xl font-semibold transition-all duration-300 ${
                        router.pathname === '/post' 
                    ...
                    <button 
                      onClick={() => { router.push('/favorites'); setMobileMenuOpen(false) }} 
                      className={`flex items-center space-x-3 w-full text-left px-4 py-3 rounded-xl font-semibold transition-all duration-300 ${
                        router.pathname === '/favorites' 
      ...
      <main className="relative">
        ...
        {user && router.pathname === '/' && (
          <button
            ...
            title={safeT('ui.listProperty', 'List Your Property')}
          >
        ...
        <button
          ...
          title={safeT('ui.scrollToTop', 'Scroll to Top')}
        >
          <span className="text-xl">↑</span>
        </button>
      </main>
      
      {!router.pathname.startsWith('/chat') && !router.pathname.startsWith('/messages') && (
      <footer className="bg-gradient-to-r from-gray-900 to-blue-900 text-white mt-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
            <div className="col-span-2">
              ...
              <p className="text-gray-300 mb-6 text-lg leading-relaxed">
                {safeT('footer.description', 'Premier house rental platform. Find your perfect home across major cities with trust and ease.')}
              </p>
            ...
            <div>
              <h4 className="font-bold text-white mb-6 text-lg">{safeT('footer.quickLinks', 'Quick Links')}</h4>
              <ul className="space-y-3 text-gray-300">
                <li>
                  <button onClick={() => router.push('/')} className="hover:text-yellow-400 transition-colors duration-200 flex items-center space-x-2">
                    <span>🏠</span><span>{safeT('nav.home', 'Home')}</span>
                  </button>
                </li>
                <li>
                  <button onClick={() => router.push('/post')} className="hover:text-yellow-400 transition-colors duration-200 flex items-center space-x-2">
                    <span>📝</span><span>{safeT('nav.addProperty', 'Add Property')}</span>
                  </button>
                </li>
                {user && (
                  <li>
                    <button onClick={() => router.push('/profile')} className="hover:text-yellow-400 transition-colors duration-200 flex items-center space-x-2">
                      <span>👤</span><span>{safeT('nav.profile', 'Profile')}</span>
                    </button>
                  </li>
                )}
              </ul>
            ...
            <div>
              <h4 className="font-bold text-white mb-6 text-lg">{safeT('footer.contact', 'Contact')}</h4>
          ...
          <div className="border-t border-gray-700 mt-12 pt-8 text-center">
            <p className="text-gray-300 text-lg">&copy; 2025 Bashfield. {safeT('footer.allRights', 'All rights reserved.')}</p>
            <p className="text-yellow-400 mt-2 font-semibold">{safeT('footer.madeWith', 'Made with ❤️ for everyone')}</p>
          </div>
        </div>
      </footer>
      )}
