  ...
  const addMarkersToMap = () => {
    ...
    clusters.forEach((cluster) => {
      if (cluster.listings.length === 1) {
        ...
        const formattedPrice = listing.price >= 1000 
          ? `${Math.round(listing.price / 1000)}k` 
          : listing.price.toString()
        
        const markerHtml = `
          <div class="bg-white border-2 border-blue-500 rounded-lg px-2 py-1 shadow-lg text-xs font-bold text-blue-600 whitespace-nowrap max-w-20 overflow-hidden">
        ...
        const minFormatted = minPrice >= 1000 ? `${Math.round(minPrice / 1000)}k` : minPrice.toString()
        const maxFormatted = maxPrice >= 1000 ? `${Math.round(maxPrice / 1000)}k` : maxPrice.toString()
