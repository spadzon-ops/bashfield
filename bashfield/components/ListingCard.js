...
export default function ListingCard({
  ...
  useEffect(() => {
    ...
    const translateListing = async () => {
      const translated = {
        ...listing,
        title: await translateContent(listing.title),
        description: await translateContent(listing.description),
        city: await translateContent(listing.city)
      }
      setTranslatedListing(translated)
    }
    translateListing()
  }, [listing, translateContent])
  ...
  const openWhatsApp = (e) => {
    e.stopPropagation()
    const message = `Hi! I'm interested in your property: ${translatedListing.title}`
              ...
              <div className="flex items-center justify-between">
                <div className="text-right">
                  <div className="text-2xl md:text-3xl font-bold bg-gradient-to-r from-blue-600 to-blue-700 bg-clip-text text-transparent">
                    {listing.price.toLocaleString()} {listing.currency} <span className="text-sm text-gray-500 font-normal">per month</span>
                  </div>
                </div>
              </div>
            </div>
              ...
              <div className="flex items-center space-x-4 flex-wrap gap-2">
                ...
                <div className="flex items-center space-x-2 bg-gray-50 rounded-xl px-3 py-2">
                  ...
                  <span className="text-sm font-semibold text-gray-700">
                    {translatedListing.city?.charAt(0).toUpperCase() + translatedListing.city?.slice(1)}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between">
              <div className="text-right">
                <div className="text-xl sm:text-2xl font-bold bg-gradient-to-r from-blue-600 to-blue-700 bg-clip-text text-transparent">
                  {listing.price.toLocaleString()} {listing.currency} <span className="text-xs text-gray-500 font-normal">per month</span>
                </div>
              </div>
            </div>
          </div>
