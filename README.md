...
## Big Update Notes
- New Navbar with language switcher and Gmail-only auth enforcement.
- Messages inbox with live updates.
- Profile avatar upload to Supabase `house-images` bucket.
- Search + pagination on home.
- SEO routes: `/api/robots.txt`, `/api/sitemap.xml`.
- Caching headers for public images.
- Centralized Supabase client at `lib/supabaseClient.js`.
